...
def phong_shading(vertices, normals, triangles, image_size=(512, 512)):
        ...
        for x in range(int(xmin), int(xmax) + 1):
            for y in range(int(ymin), int(ymax) + 1):
                ...
                if u >= 0 and v >= 0 and w >= 0:  # Point inside the triangle
                    ...
                    color = calculate_lighting(normal, light_pos, np.array([x, y, 0]), material, light_intensity)
                    pixels[x, y] = tuple(np.clip(color, 0, 255).astype(int))
...
normals = np.zeros_like(transformed_vertices)
for i, triangle in enumerate(triangles):
    normal = calculate_triangle_normal(transformed_vertices, triangle)
...
# Phong shading with depth buffer
image_phong_shading = phong_shading(transformed_vertices, normals, triangles, image_size=(512, 512))
image_phong_shading.save("rasterized_sphere_with_phong_shading.png")
