...
# Scene and antialiasing configuration
image_width, image_height = 512, 512
samples_per_pixel = 64  # N
...
# Modified rendering loop with antialiasing
...
for x in range(image_width):
    for y in range(image_height):
        ...
        for sample in range(samples_per_pixel):
            # Generate random point within pixel for antialiasing
            u_offset = np.random.uniform(-0.5, 0.5)
            v_offset = np.random.uniform(-0.5, 0.5)
            u = l + (r - l) * (x + u_offset) / image_width
            v = b + (t - b) * (y + v_offset) / image_height
            ...
            if closest_object is not None:
                ...
                accumulated_color += np.array([0, 0, 0])  # Background color

        # Average the color by the number of samples
        average_color = accumulated_color / samples_per_pixel
        image[y, x] = average_color * 255

# Convert the float image array to uint8 and save
image = np.flipud(image)
image = image.astype(np.uint8)
img = Image.fromarray(image, 'RGB')
img_path = "ray_tracing_antialiasing.png"
img.save(img_path)
img.show()
